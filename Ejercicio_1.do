...
**-- Se genera la variable principal de matrícula universitaria por tipo de institución
	...
	codebook p306 p307d p308a p308d /*matricula,*/
...
**-- Se realizan los cálculos para la población en situación de pobreza (pobres no extremos y pobres extremos)
    ...
    codebook pobreza
	tab matri_univ [aw=facpob]
    tab matri_univ if (pobreza==1 | pobreza==2) [aw=facpob]
...
**-- Se exportan las tablas
	...
	putexcel `locletter2'`row':`locletter7'`row'="Muestra: Población", merge hcenter
	putexcel `locletter8'`row':`locletter13'`row'="Muestra: Matriculados actualmente", merge hcenter
	...
	forvalues x=1/3 {
	tab matri_univ if region==`x' & (pobreza==1 | pobreza==2) [aw=facpob], matcell(freq) matrow(names)
	...
	forvalues x=1/3 {
	tab matri_univ_b if region==`x' & (pobreza==1 | pobreza==2) [aw=facpob], matcell(freq) matrow(names)
	local rows = rowsof(names)	
	forvalues i = 1/`rows' {
        local freq_val = freq[`i',1]
        local percent_val = `freq_val'/`r(N)'*100
	...
	tab matri_univ if (pobreza==1 | pobreza==2) [aw=facpob], matcell(freq) matrow(names)
	local rows = rowsof(names)
	...
	tab matri_univ_b if (pobreza==1 | pobreza==2) [aw=facpob], matcell(freq) matrow(names)
	local rows = rowsof(names)
    local row = `row' + 1
	forvalues i = 1/`rows' {
        local val = names[`i',1]
